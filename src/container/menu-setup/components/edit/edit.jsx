...
class EditDialog extends Component {
    ...
    handleOk = ()=>{
        this.props.form.validateFields((err, values) => {
            if (!err) {
                // const {menuName, menuPath, menuParams, menuIcon, menuSort, hidden} = values;
                            ...
                            <Select.OptGroup label={itemGroup.category} key={indexGroup}>
                                {
                                    ...
                                    : itemGroup.icons.map((item,index)=>{
                                        return (
                                            <Select.Option value={itemGroup.theme + item.type} key={index}><Icon type={item.type} key={index} theme={itemGroup.theme} /> {item.type}</Select.Option>
                                        )
                                    })
                                }
                            </Select.OptGroup>
    ...
    render() {
        ...
        this.parentNode = this.state.parentMenuList.length===0 ? null : (
            <Select placeholder='请选择上级菜单'>
                {
                    this.state.parentMenuList.map((item,index)=>{
                        return (
                    ...
                    <Form {...formItemLayout}>
                        ...
                        <Item label='菜单路径'>
                            {getFieldDecorator('menuPath',{
                                initialValue:this.props.menuObj.path,
